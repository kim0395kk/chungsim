        ...
        if 'final_doc' in st.session_state:
            ...
            if doc:
                ...
                <div class="paper-sheet">
                    ...
                    <div class="doc-header">{_sanitize_text(doc.get('title', '공 문 서'))}</div>
                    <div class="doc-info">
                        <span>문서번호: {_sanitize_text(meta.get('doc_num'))}</span>
                        <span>시행일자: {_sanitize_text(meta.get('today_str'))}</span>
                        <span>수신: {_sanitize_text(doc.get('receiver', '수신자 참조'))}</span>
                    </div>
                ...
                for p in paragraphs:
                    html_content += f"<p style='margin-bottom: 15px;'>{_sanitize_text(p)}</p>"
                html_content += f"""
                    </div>
                    <div class="doc-footer">{_sanitize_text(doc.get('department_head', '행정기관장'))}</div>
                </div>
                """
