# streamlit_app.py
# ✅ 완세트 (414 방어 + law_api_service 미정의 해결 + google.genai 마이그레이션 + secrets 방어)
#
# requirements.txt 예시
# streamlit
# google-genai
...
def _get_secret(section: str, key: str, default=None):
    """st.secrets 방어: 키/섹션 없어도 앱이 죽지 않게."""
    try:
...
class LLMService:
    ...
    def generate_json(self, prompt: str, schema=None):
        # schema 파라미터는 호환용(기존 호출 유지). 실제로는 "엄격 JSON" 강제 + 파싱.
        strict = (
            prompt
            + "\n\n"
...
출력 예: 도로교통법 제32조 주정차 금지 과태료 의견제출 행정절차법
"""
        try:
...
[역할] 베테랑 행정 공무원
[상황] "{situation}"
[임무] 아래 검색 후보 중 '가장 신뢰할 수 있고 실무에 도움이 되는 자료' 순서로 인덱스만 JSON 리스트로 출력.
...
class LegalAgents:
    ...
    def researcher(situation):
        """
        [업그레이드된 하이브리드 리서치]
        - 실체법 + 정의/반박 + 절차법(행정절차법/질서위반행위규제법 등)까지
        - category를 붙여 strategist가 절차법을 '핵심 주의사항'에 매핑하기 쉽게 함
        """
        prompt_extract = f"""
상황: "{situation}"

위 민원을 법적으로 완벽하게 처리하기 위해 필요한 법령과 조문을
아래 카테고리에 맞춰 최대 4개까지 JSON 리스트로 추출하시오.
...
"""
        ...
        report_lines = []
        api_success_count = 0
        report_lines.append(f"🔍 **AI가 식별한 핵심 법령 (실체+절차) {len(search_targets)}건**")
        report_lines.append("---")
...
[AI 추론 결과]
Role: 행정 법률 전문가
Task: "{situation}" 상황에 필요한 실체법과 절차법을 찾아 설명하시오.
주의: 법령 API 연결 실패 상태이므로, 환각 가능성 경고를 포함하고 '법제처 확인 필수'를 명시하시오.
JSON 금지, 일반 텍스트로 작성.
"""
    ...
    def strategist(situation, legal_basis, search_results):
        """
        절차법까지 포함해서 '핵심 주의사항'에 반드시 인용하도록 강제
        """
        prompt = f"""
...
[임무]
아래 양식(마크다운)에 맞춰 '공무원 보고서' 스타일로 작성하세요.
특히 **핵심 주의사항**은 반드시 '절차 관련 법령(행정절차법/질서위반행위규제법 등)'을 인용해 매핑해야 합니다.
...
2) **핵심 주의사항(절차 매핑 필수)**
- 3~4개 항목
- 각 항목 끝에 근거 법령/조문을 반드시 표기
  예: 사전통지 및 의견제출 기회 부여 (행정절차법 제21조)
...
위 상황에서 사전통지/이행명령/의견제출 등 절차상 통상 부여하는 기간(일수)을 숫자만 출력.
(예: 10, 15, 20) 모르면 15.
"""
        try:
...
# ==========================================
# 2.5 Global service instances (⭐ 이게 law_api_service 미정의 해결)
# ==========================================
...
def main():
    ...
    with col_left:
        ...
        if not search_service.client_id or not search_service.client_secret:
            warn_lines.append("- 네이버 검색 키 없음: `NAVER_CLIENT_ID/SECRET` 없으면 유사사례 검색이 꺼집니다.")
        if warn_lines:
            st.warning("현재 설정 상태:\n" + "\n".join(warn_lines))
